...
export function buildStandalonePrintHtml(
  ...
  // on-screen wrappers (max-w-5xl / max-w-4xl) snapshot as pixel widths
  // that exceed the printable area once rendered at driver DPI, so the
  // container overrides are !important.
  const containerCss = getPrintContainerStyle(paperSize);
  const existing = clone.getAttribute("style") ?? "";
  const forced = containerCss
    .split(";")
    .filter(Boolean)
