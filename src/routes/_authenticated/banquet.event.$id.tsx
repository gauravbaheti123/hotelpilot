...
function BanquetEventPage() {
  ...
  const [fin, setFin] = useState<{ advance: number; balance: number; hasFolio: boolean } | null>(
    null,
  );
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      loadEventFinancials(ev.booking_id)
        .then(setFin)
        .catch(() => setFin(null));
    ...
    const nextDay = new Date(bq.event_date);
  ...
  async function addRoomBlock() {
    ...
    const { error } = await supabase.from("event_room_blocks").insert({
      ...
      event_name: b.event_name ?? eventRefShort(b.banquet_number, b.booking_number),
      room_id: roomRow!.id,
  ...
  async function doBulkCheckIn() {
    ...
    setBulkBusy(true);
    try {
      const res = await bulkCheckInBlocks({
  ...
  async function setStatus(next: "confirmed" | "in_progress" | "completed") {
    ...
    if (next === "completed") {
      ...
      if (occ > 0) {
        return toast.error(
          `${occ} room(s) still occupied. Checkout guests before completing event.`,
        );
      }
    }
    if (!ids) return;
  ...
  async function saveMeta() {
    if (!b || !ids) return;
    if (!meta.event_date || !meta.start_time || !meta.end_time)
  ...
  async function patchCharges(patch: Partial<Bq>) {
    ...
    if (Object.prototype.hasOwnProperty.call(patch, "discount_amount")) {
      const sub =
        ...
        Number(merged.hall_charge) +
        Number(merged.fb_charge) +
        Number(merged.extra_charge);
    ...
    const total = computeBanquetTotal({
      package_rate: merged.package_rate,
      pax: merged.pax,
      hall_charge: merged.hall_charge,
      fb_charge: merged.fb_charge,
      extra_charge: merged.extra_charge,
  ...
  async function deleteEvent() {
    ...
    try {
      ...
      await logActivity({
        ...
        reference_label: `${eventRefShort(b.banquet_number, b.booking_number)} — ${b.host_name ?? b.guests?.name ?? ""}`,
        details: {
          event_id: ids.bookingId,
          banquet_number: eventRefShort(b.banquet_number, b.booking_number),
      ...
      toast.success(`Event ${eventRefShort(b.banquet_number, b.booking_number)} permanently deleted`);
  ...
  return (
    <AppShell title={`Banquet ${eventRef(b.banquet_number, b.booking_number)}`}>
      <div className="max-w-6xl space-y-4">
        <div className="flex flex-wrap items-center gap-3">
          <BackButton fallbackTo="/banquet/bookings" />
          <Badge variant="outline" className={BANQUET_STATUS_TONE[b.status]}>
            {b.status.toUpperCase()}
          </Badge>
          <div className="text-sm text-muted-foreground">
            {b.event_name ? (
              <span className="font-medium text-foreground mr-1">{b.event_name} ·</span>
            ) : null}
            {b.halls?.name ?? "—"} · {b.event_date} {b.start_time?.slice(0, 5)} →{" "}
            {b.event_end_date && b.event_end_date !== b.event_date ? `${b.event_end_date} ` : ""}
            {b.end_time?.slice(0, 5)} · {b.pax} pax
          </div>
          {editable && (
            ...
            <div className="flex gap-2">
              ...
              {b.status === "in_progress" && (
                <Button size="sm" onClick={() => setStatus("completed")}>
                  <CheckCircle2 className="h-4 w-4 mr-1" /> Complete Event
                </Button>
              )}
              <Button
                size="sm"
        ...
        <div className="grid gap-4 lg:grid-cols-2">
              ...
              <div className="grid gap-3 sm:grid-cols-2">
                ...
                <div className="space-y-1.5">
                  ...
                  <Input
                    ...
                    disabled={!editable}
                    onChange={(e) =>
                      setHost({ ...host, mobile: e.target.value.replace(/[^\d+\-\s]/g, "") })
                    }
                  />
                </div>
                ...
                <div className="space-y-1.5 sm:col-span-2">
                  ...
                  <Select
                    value={host.function_type}
                    onValueChange={(v) => setHost({ ...host, function_type: v })}
                    disabled={!editable}
                  >
                    <SelectTrigger>
                ...
                <div className="space-y-1.5 sm:col-span-2">
                  ...
                  <Textarea
                    ...
                    value={host.notes}
                    disabled={!editable}
                    onChange={(e) => setHost({ ...host, notes: e.target.value })}
                  />
                </div>
              </div>
              {editable && (
                <div className="pt-1">
                  <Button size="sm" onClick={saveHost} disabled={savingHost}>
                    <Save className="h-3.5 w-3.5 mr-1" /> Save Host Details
                  </Button>
                </div>
              )}
            </CardContent>
          ...
          <Card>
            ...
            <CardContent className="grid gap-3 sm:grid-cols-2">
              <NumField
                label="Package / pax"
                value={b.package_rate}
                onSave={(v) => patchCharges({ package_rate: v })}
                disabled={!editable}
              />
              <NumField
                label="Pax"
                value={b.pax}
                onSave={(v) => patchCharges({ pax: v })}
                disabled={!editable}
              />
              <NumField
                label="Hall"
                value={b.hall_charge}
                onSave={(v) => patchCharges({ hall_charge: v })}
                disabled={!editable}
              />
              <NumField
                label="F&B"
                value={b.fb_charge}
                onSave={(v) => patchCharges({ fb_charge: v })}
                disabled={!editable}
              />
              <NumField
                label="Extra"
                value={b.extra_charge}
                onSave={(v) => patchCharges({ extra_charge: v })}
                disabled={!editable}
              />
              <div className="space-y-1.5">
                <Label className="text-xs">Extra description</Label>
                <Input
                  value={b.extra_charge_description ?? ""}
                  disabled={!editable}
                  onChange={(e) => setB({ ...b, extra_charge_description: e.target.value })}
                  onBlur={(e) =>
                    patchCharges({ extra_charge_description: e.target.value || null } as any)
                  }
                />
              </div>
              <NumField
                label="Discount"
                value={b.discount_amount}
                onSave={(v) => patchCharges({ discount_amount: v })}
                disabled={!editable}
              />
              <NumField
                label="Advance"
                value={b.advance_amount}
                onSave={(v) => patchCharges({ advance_amount: v })}
                disabled={!editable}
              />
              <div className="space-y-1.5">
                <Label className="text-xs">Advance mode</Label>
                <Select
                  value={b.advance_payment_mode ?? ""}
                  disabled={!editable}
              ...
              <div className="space-y-1.5 sm:col-span-2 border-t pt-2 text-sm">
                ...
                {fin?.hasFolio ? (
                  <>
                    ...
                    <Row
                      k="Balance"
                      v={`₹${fin.balance.toLocaleString("en-IN")}`}
                      bold
                      highlight={fin.balance > 0}
                    />
                  </>
                ) : (
                  <Row
                    k="Balance"
                    v={`₹${Number(b.balance_amount).toLocaleString("en-IN")}`}
                    bold
                    highlight={Number(b.balance_amount) > 0}
                  />
                )}
              </div>
            </CardContent>
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between">
            <CardTitle className="text-base flex items-center gap-2">
              <BedDouble className="h-4 w-4" /> Rooms · Assign Guest
            </CardTitle>
            <div className="flex gap-2">
              ...
              {blocks.some((bk) => bk.status === "checked_out") && (
                <Button
                  ...
                  variant="outline"
                  onClick={() =>
                    router.navigate({ to: "/banquet/master-bill/$id", params: { id: b.id } })
                  }
                >
              ...
              {editable && (
                <Button size="sm" onClick={() => setAddOpen(true)}>
                  <Plus className="h-4 w-4 mr-1" /> Assign Guest / Block Room
                </Button>
              )}
            </div>
          </CardHeader>
              ...
              <div className="space-y-2">
                ...
                {blocks.map((blk, i) => {
                  ...
                  return (
                    <div
                      key={blk.id}
                      className="grid grid-cols-[70px_100px_minmax(230px,1.2fr)_1fr_130px_110px_220px] gap-2 items-center text-sm"
                    >
                      ...
                      <div className="space-y-1">
                        <div className="flex items-center gap-1">
                          ...
                          <Input
                            type="date"
                            className="h-8 w-[130px] px-1 text-xs"
                            disabled={!isBlocked}
                          ...
                          <Input
                            type="time"
                            className="h-8 w-[92px] px-1 text-xs"
                            disabled={!isBlocked}
                      ...
                      <GuestSearchInput
                        ...
                        onSelect={(g) => {
                          const next = {
                            ...blocks[i],
                            guest_name: g.name,
                            guest_mobile: g.mobile ?? blocks[i].guest_mobile,
                          };
                          patchBlockLocal(i, {
                            guest_name: next.guest_name,
                            guest_mobile: next.guest_mobile,
                          });
                          saveBlockGuest(next);
                        }}
                      ...
                      <Input
                        ...
                        onChange={(e) =>
                          patchBlockLocal(i, { guest_mobile: sanitizeMobile(e.target.value) })
                        }
                      ...
                      <div className="flex flex-wrap gap-1">
                        {isBlocked && !hasGuest && (
                          <Button size="sm" variant="outline" onClick={() => openAssign(blk)}>
                            <UserPlus className="h-3.5 w-3.5 mr-1" />
                            Assign
                          </Button>
                        )}
                        {isBlocked && hasGuest && (
                          <Button size="sm" onClick={() => doCheckIn(blk)}>
                        ...
                        {blk.status === "checked_in" && (
                          <>
                            ...
                            <Button size="sm" variant="outline" onClick={() => doCheckOut(blk)}>
        ...
        <Dialog open={addOpen} onOpenChange={setAddOpen}>
          <DialogContent>
            <DialogHeader>
              <DialogTitle>Assign guest / block room</DialogTitle>
            </DialogHeader>
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-2">
                ...
                <div className="space-y-1.5">
                  ...
                  <Input value={addGuestName} onChange={(e) => setAddGuestName(e.target.value)} />
                </div>
                <div className="space-y-1.5">
                  <Label className="text-xs">Guest mobile (optional)</Label>
                  <Input
                    value={addGuestMobile}
                    onChange={(e) => setAddGuestMobile(e.target.value)}
                  />
                </div>
              </div>
            </div>
          ...
          <DialogContent>
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              ...
              <div className="space-y-1.5 sm:col-span-2">
                ...
                <Select
                  value={meta.hall_id}
                  onValueChange={(v) => setMeta({ ...meta, hall_id: v })}
                >
                  <SelectTrigger>
                ...
                <Input
                  ...
                  onChange={(e) => {
                    ...
                    setMeta((m) => ({
                      ...
                      event_date: v,
                      event_end_date:
                        !m.event_end_date || m.event_end_date < v ? v : m.event_end_date,
                    }));
                  }}
                />
              </div>
              <div className="space-y-1.5">
                ...
                <Input
                  ...
                  onChange={(e) => setMeta({ ...meta, event_end_date: e.target.value })}
                />
              </div>
              <div className="space-y-1.5">
                ...
                <Input
                  type="number"
                  value={meta.pax}
                  onChange={(e) => setMeta({ ...meta, pax: Number(e.target.value) })}
                />
              </div>
              <div className="space-y-1.5">
                ...
                <Input
                  ...
                  onChange={(e) => setMeta({ ...meta, start_time: e.target.value })}
                />
              </div>
              <div className="space-y-1.5">
                ...
                <Input
                  type="time"
                  value={meta.end_time}
                  onChange={(e) => setMeta({ ...meta, end_time: e.target.value })}
                />
              </div>
              <div className="space-y-1.5 sm:col-span-2">
                <Label className="text-xs">Function type</Label>
                <Select
                  value={meta.function_type}
                  onValueChange={(v) => setMeta({ ...meta, function_type: v })}
                >
                  <SelectTrigger>
        ...
        <Dialog open={assignOpen} onOpenChange={setAssignOpen}>
          <DialogContent>
            <DialogHeader>
              <DialogTitle>Assign guest to room {assignBlock?.room_number}</DialogTitle>
            </DialogHeader>
            <div className="space-y-3">
              <div className="space-y-1.5">
                ...
                <GuestSearchInput
                  ...
                  onSelect={(g) => {
                    setAssignName(g.name);
                    if (g.mobile) setAssignMobile(g.mobile);
                  }}
                />
              </div>
              <div className="space-y-1.5">
                ...
                <Input
                  ...
                  maxLength={10}
                  value={assignMobile}
                  onChange={(e) => setAssignMobile(sanitizeMobile(e.target.value))}
                />
              </div>
            </div>
            <DialogFooter>
        ...
        >
          <DialogContent>
            ...
            <p className="text-sm text-muted-foreground">
              {eventRefShort(b.banquet_number, b.booking_number)} and its bill lines will be removed for good. A full snapshot is
      ...
      <CheckoutDialog
        ...
        onOpenChange={(o) => {
          if (!o) setCheckoutBookingId(null);
        }}
        onDone={() => {
