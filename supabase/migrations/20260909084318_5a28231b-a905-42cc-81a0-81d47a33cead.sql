...
BEGIN
  ...
  -- Remember whether the old room actually had a live room charge; only then
  -- does a missing new charge represent a billing loss.
  ...
  IF v_mode = 'same_day' THEN
    ...
    IF v_new_charge IS NULL THEN
      IF v_old_charge IS NOT NULL THEN
        -- Never finish quietly with the OLD room still on the bill.
