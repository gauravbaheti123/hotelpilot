...
BEGIN
  ...
  -- Rooms: prefer the multi-room `rooms` array; fall back to the legacy
  -- single-room top-level fields so existing callers keep working.
  ...
  VALUES (
    ...
    jsonb_build_object(
      ...
      'check_out', payload->>'check_out',
      'room_id', _first_room,
      'unassigned', (_first_room IS NULL),
      'rooms', jsonb_array_length(_rooms),
