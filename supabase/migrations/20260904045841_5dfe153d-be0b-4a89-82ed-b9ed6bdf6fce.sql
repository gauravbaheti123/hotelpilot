CREATE OR REPLACE FUNCTION public.owner_update_guest_name(
  _guest_id uuid, _name text, _reason text
) RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
CREATE OR REPLACE FUNCTION public.owner_update_booking_room_details(
  _booking_room_id uuid, _room_id uuid, _category_id uuid,
  _check_in date, _check_out date, _reason text
) RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
...
REVOKE ALL ON FUNCTION public.owner_update_guest_name(uuid, text, text) FROM PUBLIC, anon;
REVOKE ALL ON FUNCTION public.owner_update_booking_room_details(uuid, uuid, uuid, date, date, text) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.owner_update_guest_name(uuid, text, text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.owner_update_booking_room_details(uuid, uuid, uuid, date, date, text) TO authenticated;