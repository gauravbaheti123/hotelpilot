...
AS $function$
  SELECT COALESCE(
    (SELECT bs.prefix FROM public.bill_sequences bs
      WHERE bs.property_id = _property_id AND bs.sequence_type = _segment),
...
BEGIN
  ...
  -- Stored prefix is authoritative when the row exists: an explicitly empty
  -- string means "no prefix". Only fall back to short_code reconstruction when
  -- the sequence row is missing or its prefix was never set (NULL).
