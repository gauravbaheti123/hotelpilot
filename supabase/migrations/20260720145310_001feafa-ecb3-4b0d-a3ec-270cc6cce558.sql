-- Tiered room GST: 0% ≤ ₹1000, 5% ≤ ₹7500, 18% > ₹7500 (per Sept 2025 rule)

-- 1. Reset default gst_slabs rows for all properties to the correct tiers
UPDATE public.gst_slabs SET gst_rate = 0  WHERE from_amount = 0    AND to_amount = 1000;
UPDATE public.gst_slabs SET gst_rate = 5  WHERE from_amount = 1001 AND to_amount = 7500;
UPDATE public.gst_slabs SET gst_rate = 18 WHERE from_amount = 7501;

-- 2. Helper: nightly rate → GST% (rooms only)
CREATE OR REPLACE FUNCTION public.room_gst_rate_for_tariff(_rate numeric)
RETURNS numeric
LANGUAGE sql IMMUTABLE
SET search_path TO 'public'
AS $$
  SELECT CASE
    WHEN COALESCE(_rate,0) <= 1000 THEN 0
    WHEN _rate <= 7500 THEN 5
    ELSE 18
  END::numeric;
$$;

-- 3. Rewrite seed_room_charge to use tiered rate
...
BEGIN
  ...
  -- For inclusive tariff, threshold is measured on the taxable (pre-GST) per-night value.
  -- Solve for taxable: nightly = taxable * (1 + rate/100). Try 5% first; if taxable > 7500 use 18%.
  IF COALESCE(v_booking.rate_type, 'exclusive') = 'inclusive' THEN
    IF v_nightly <= 1000 THEN
      v_gst_rate := 0;
    ELSIF v_nightly / 1.05 <= 7500 THEN
      v_gst_rate := 5;
    ELSE
      v_gst_rate := 18;
    END IF;
  ...
  ELSE
    v_gst_rate   := public.room_gst_rate_for_tariff(v_nightly);
    v_gross      := v_nights * v_nightly;
    v_amount     := v_gross;
...
-- 4. Rewrite post_nightly_room_charges to use tiered rate
...
DECLARE
  ...
  v_gst_rate numeric; v_amount numeric; v_gst numeric;
BEGIN
  ...
  LOOP
    ...
    IF v_rec.rate_type = 'inclusive' THEN
      IF v_rec.rate <= 1000 THEN v_gst_rate := 0;
      ELSIF v_rec.rate / 1.05 <= 7500 THEN v_gst_rate := 5;
      ELSE v_gst_rate := 18;
      END IF;
      v_amount := ROUND((v_rec.rate / (1 + v_gst_rate / 100))::numeric, 2);
      v_gst    := ROUND((v_rec.rate - v_amount)::numeric, 2);
    ELSE
      v_gst_rate := public.room_gst_rate_for_tariff(v_rec.rate);
      v_amount   := v_rec.rate;
...
-- 5. Backfill open (non-settled, non-locked) room charges to the tiered rate
UPDATE public.folio_charges fc
   SET gst_rate = public.room_gst_rate_for_tariff(fc.rate),
       gst_amount = ROUND((fc.amount * public.room_gst_rate_for_tariff(fc.rate) / 100)::numeric, 2)
  FROM public.folios f
 WHERE fc.folio_id = f.id
   AND fc.charge_type = 'room'
   AND COALESCE(fc.is_wiped,false) = false
   AND COALESCE(fc.gst_rate,0) <> public.room_gst_rate_for_tariff(fc.rate)
   AND f.status NOT IN ('settled','void')
   AND COALESCE(f.is_deleted,false) = false
   AND NOT public.is_day_locked(f.property_id, COALESCE(fc.charged_on, CURRENT_DATE));

-- Recompute open folio totals
DO $$
DECLARE r record;
BEGIN
  FOR r IN
