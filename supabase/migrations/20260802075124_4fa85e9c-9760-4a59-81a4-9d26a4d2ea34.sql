-- 1) Make the shared bill-number generator self-healing and collision-proof.
...
BEGIN
  ...
  -- Lock the counter row for the rest of this transaction.
  PERFORM 1 FROM public.bill_sequences
   WHERE property_id = _property_id AND sequence_type = _segment
   FOR UPDATE;

  -- Self-heal: never issue a number at or below one already consumed.
  IF _segment = 'banquet' THEN
    SELECT COALESCE(MAX(NULLIF(substring(banquet_number from '([0-9]+)$'),'')::int), 0)
      INTO v_max
...
-- 2) Banquet numbering now always uses the shared generator (no legacy EVENT### path).
...
AS $function$
BEGIN
  IF NEW.banquet_number IS NOT NULL AND btrim(NEW.banquet_number) <> '' THEN
    RETURN NEW;
  END IF;
...
-- 3) Remove diagnostic test events created during this investigation.
DELETE FROM public.banquet_bookings
 WHERE function_type = 'Test' AND event_date = DATE '2030-01-01';
