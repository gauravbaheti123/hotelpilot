-- fix: rooms.status is an enum (room_status) — cast to text for the return type
...
AS $$
  SELECT r.id, r.room_number, r.category_id, r.status::text, r.floor
  FROM public.rooms r
  WHERE r.property_id = _property_id
    AND r.status::text = 'vacant'
...
-- fix: bookings.status is booking_status enum, rooms.status is room_status enum
