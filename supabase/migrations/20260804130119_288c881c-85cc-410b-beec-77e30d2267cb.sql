-- =========================================================
-- 1) available_rooms(_property_id, _check_in, _check_out, _category_id)
-- =========================================================
...
AS $$
  SELECT r.id, r.room_number, r.category_id, r.status, r.floor
  FROM public.rooms r
  WHERE r.property_id = _property_id
    AND r.status = 'vacant'
    AND (_category_id IS NULL OR r.category_id = _category_id)
    AND public.has_permission(auth.uid(), _property_id, 'bookings', 'view')
    -- not already booked for an overlapping range
    AND NOT EXISTS (
      SELECT 1
      ...
      WHERE br.room_id = r.id
        ...
        AND br.check_in < _check_out
        AND br.check_out > _check_in
        AND br.actual_check_out IS NULL
    )
    -- not blocked by a banquet event for an overlapping range
    AND NOT EXISTS (
      SELECT 1
      FROM public.event_room_blocks eb
      WHERE eb.room_id = r.id
        AND eb.status IN ('blocked', 'checked_in')
        AND eb.checkin_date < _check_out
        AND eb.checkout_date > _check_in
    )
...
-- =========================================================
-- 2) create_booking(payload jsonb) — single transactional write path
-- =========================================================
...
BEGIN
  ...
  -- ---------- Guest (single "nation" drives nationality + country) ----------
  _nation := NULLIF(btrim(COALESCE(g->>'nation', '')), '');
  _mobile := NULLIF(btrim(COALESCE(g->>'mobile', '')), '');
  _guest_id := NULLIF(g->>'guest_id', '')::uuid;

  -- Duplicate detection, same precedence as lookupExistingGuestId():
  -- exact 10-digit mobile first, then id_proof_number.
  ...
  INSERT INTO public.bookings (
    ...
    CASE WHEN _check_in_now THEN 'checked_in' ELSE 'reserved' END,
  ...
  IF _check_in_now AND _effective_room IS NOT NULL THEN
    UPDATE public.rooms SET status = 'occupied' WHERE id = _effective_room;
  END IF;

  -- ---------- Event room block sync (banquet handoff) ----------
