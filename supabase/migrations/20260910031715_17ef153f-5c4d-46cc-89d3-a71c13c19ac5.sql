-- 1) Repair the affected stay row (guest is still in-house)
SELECT set_config('app.allow_actual_time_edit', 'true', true);

UPDATE public.booking_rooms br
   SET actual_check_out = NULL,
       updated_at = now()
 WHERE br.id = '278ee8a8-47ed-4ee5-bfae-e8550e078f41'
   AND EXISTS (
     SELECT 1 FROM public.bookings b
      WHERE b.id = br.booking_id AND b.status = 'checked_in'
   );

SELECT set_config('app.allow_actual_time_edit', '', true);

UPDATE public.rooms
   SET status = 'occupied', updated_at = now()
 WHERE id = '7a98ccd3-be21-4aaa-8bac-17097b8fcc81'
   AND status <> 'maintenance';

-- 2) Guard: never let an owner override stamp a check-out time on an in-house stay
...
BEGIN
  ...
  -- An in-house guest must be closed through the Check-out flow. Stamping an
  -- actual check-out here leaves the booking checked in while the room drops
  -- off the dashboard grid (which reads live stay rows with no check-out).
  IF _actual_check_out IS NOT NULL
     AND (_old->>'actual_check_out') IS NULL
     AND _booking_status IN ('reserved', 'checked_in') THEN
    RAISE EXCEPTION 'This guest has not checked out yet — use Check-out to close the stay.';
  END IF;
