...
-- 1. Master-data allowance fields
ALTER TABLE public.room_categories
  ADD COLUMN IF NOT EXISTS complimentary_food_limit_per_person numeric(12,2) NOT NULL DEFAULT 0;

ALTER TABLE public.tariff_plans
  ADD COLUMN IF NOT EXISTS complimentary_food_limit_per_person numeric(12,2) NOT NULL DEFAULT 0;

-- 2. Folio running total
ALTER TABLE public.folios
  ADD COLUMN IF NOT EXISTS complimentary_food_used numeric(12,2) NOT NULL DEFAULT 0;

-- 3. Rewritten recompute with MAP/AP per-night allowance (strict reset)
...
BEGIN
  ...
  -- MAP/AP complimentary food deduction, strict per-night reset.
  -- For each night d, allowance = SUM(limit × pax) across active booking_rooms
  -- with meal_plan IN ('MAP','AP') that cover that night; deduct MIN(day_food, day_allowance).
  WITH nights AS (
    SELECT
      d.d AS night_date,
      SUM(
  ...
  -- Cap complimentary at the raw food sub-total to be safe.
  IF v_comp > 0 THEN
    SELECT COALESCE(SUM(
    ...
    -- Scale food GST proportionally to remaining (non-complimentary) food portion.
...
-- 4. Refresh totals for open folios so existing MAP/AP bookings pick up the deduction.
DO $$
DECLARE r record;
BEGIN
  FOR r IN
