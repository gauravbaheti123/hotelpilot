-- Guard folded INTO the core split routine so no client build (including an
-- older published one calling split_folio_bill directly) can cut a short bill.
CREATE OR REPLACE FUNCTION public.assert_split_conserves(_folio_id uuid, _payload jsonb)
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  -- may cover only part of the bill — they keep only the existing "must not
  -- exceed the parent" rule.
  SELECT EXISTS (
    SELECT 1
      FROM jsonb_array_elements(COALESCE(_payload->'children', '[]'::jsonb)) kid,
           jsonb_array_elements(COALESCE(kid->'charges', '[]'::jsonb)) ch
     WHERE COALESCE(ch->>'charge_type','') = 'share'
  ) INTO v_is_share;
  IF v_is_share THEN RETURN; END IF;
...
REVOKE ALL ON FUNCTION public.assert_split_conserves(uuid, jsonb) FROM anon, public;
GRANT EXECUTE ON FUNCTION public.assert_split_conserves(uuid, jsonb) TO authenticated, service_role;
...
BEGIN
  ...
  -- 0) Conservation: every live line of the original must reach the new bills.
  PERFORM public.assert_split_conserves(_folio_id, _payload);
    ...
    LOOP
      ...
      LOOP
        ...
        ELSE
          ...
          -- `trg_force_server_time_payments` (BEFORE INSERT) rewrites paid_at to
          -- now() on every insert. That is correct for normal collection, but a
          -- split must preserve the ORIGINAL receipt date, so restore it here
          -- (UPDATE is not touched by that trigger) in the same transaction.
          UPDATE public.payments
             SET paid_at = v_pay_row.paid_at
