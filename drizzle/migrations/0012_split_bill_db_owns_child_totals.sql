...
BEGIN
  ...
  -- 6) Totals are owned by the database: client-computed headers are ignored.
  FOREACH v_target IN ARRAY v_child_ids LOOP
    PERFORM public.recompute_folio_totals(v_target);
  END LOOP;

  SELECT COALESCE(SUM(total_amount),0) INTO v_sum_children
    FROM public.folios WHERE id = ANY(v_child_ids);
  IF abs(v_sum_children - COALESCE(v_parent.total_amount,0)) > 1.0 THEN
    RAISE EXCEPTION
      'Split rejected: the new bills add up to % but the original bill is %. Nothing was changed — refresh the bill and try again.',
      round(v_sum_children,2), round(COALESCE(v_parent.total_amount,0),2);
  END IF;
